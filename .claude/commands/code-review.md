...
2. Ask the reviewer worker for concise findings plus one reviewer verdict:
...
3. Post the reviewer evidence to GitHub as a pull-request review event through
   the existing governed PR-review transport:
   ...
   Use `gh api -X POST repos/<owner>/<repo>/pulls/<pr>/reviews --input -` or
   the existing governed review transport that submits the same API payload.
