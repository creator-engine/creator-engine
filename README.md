...
- The optional add-ons above (forge-automation, cockpit, containment,
  ...
  secret store.
- The package artifacts are at `creator-engine-validator` version `0.3.3`. There
...
- **`ce`** is the retained v1 command-line runtime. It wraps the validator and
  ...
  `ce update --track main` uses the fail-closed main-HEAD source-build resolver),
  `ce clean-main-install` (resolve `origin/main`, build a first-party wheel from
  that exact commit, verify source and artifact hashes, then promote only the
  verified artifact),
...
1. **Public one-liner.**
   ...
   needed, installs `creator-engine-validator==0.3.3` offline, proves `ce`,
