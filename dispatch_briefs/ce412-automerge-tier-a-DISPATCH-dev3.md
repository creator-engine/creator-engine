# DISPATCH BRIEF — dev-3 — ce-ops#412 Auto-merge Tier A: carrier/changelog split-tier
Date: 2026-07-03 · Role: implementer · Work class: XS (S at most)
Branch: `ce-412-automerge-tier-a` (off freshly-fetched origin/main; you have verified git egress — fetch first)
Worktree: create under `/var/tmp/wt-ce412-tier-a` (NOT /workspace). Venv has no activate — use `.venv/bin/python -m pytest`.

## Mandate (ratified 2026-07-03, grant R-B)
Build Tier A of the ratified auto-merge tier expansion (full proposal embedded below; YOUR scope is the Tier A section only — Tiers B/C/D are other lanes/rejected; read them only for the shared audit-line format):
1. Per-tier flag `CE_AUTOMERGE_TIER_CARRIER_CHANGELOG` (default false/absent = tier off), subordinate to the existing global `CE_AUTOMERGE_KILL_SWITCH` and run-mode gates. Follow the existing docs-class flag plumbing pattern exactly.
2. Path predicate: tier applies ONLY when every changed path is within `.ce/changelog/**` or `.ce/pr-manifests/**`. Any other path → tier inapplicable (falls through to existing behavior).
3. Audit line: tier label + NEW `reviewer_venue` field (ratified addition — records which reviewer venue/identity produced the approval; source it from the review-decision data the actuator already holds). Add the reviewer_venue field at the shared audit-record level so future tiers inherit it.
4. Baseline gates unchanged: carrier, work-class gate, full CI green, author≠approver.
5. Tests: unit coverage for the predicate (positive, negative-path, mixed-path), flag-off default, audit line contents incl. reviewer_venue.

## Files (allowed paths)
- `validators/creator_engine_validator/forge/automerge_policy.py` (tier predicate + flag)
- `validators/creator_engine_validator/forge/automerge_actuator.py` (tier re-verify + audit fields)
- `validators/creator_engine_validator/forge/mutation_classifier.py` ONLY if the predicate genuinely needs it
- `.github/workflows/automerge-decide.yml` + `.github/workflows/automerge-actuate.yml`: ENV-BLOCK LINES ONLY (materialize the new flag the same way existing flags are materialized). ⚠️ SHARED-FILE NOTICE: another in-flight lane appends a new STEP at the end of automerge-decide.yml. Confine your edit strictly to the env block; do not reorder/reformat anything else in these files.
- Matching unit tests under `validators/tests/`
- Changelog: `.ce/changelog/ce-412-automerge-tier-a.md` · Carrier: regen via carrier_gen `write_carriers(base=<merge-base>)`, never hand-edit; rm build//egg-info first.

## STOP LINES
- Do NOT flip the flag anywhere (no repo-variable changes, no default-true): flip-live is a controller act after merge.
- Do NOT touch Tier B/C/D code paths, integrator_belt.py, conveyor files, or anything outside the list above.
- No push, no PR: controller harvests. If a needed seam doesn't match the proposal's line references (they cite specific line ranges), report the discrepancy — do not improvise a new seam.

## Preflight (standing directive ce-ops#303)
FULL `ce validate-pr` (CI parity) GREEN in ONE pass before commit-for-harvest. Known env gap on this seat: ssh-keygen-dependent baseline may fail in-container — capture + mark as known env-gap; host preflight is the authoritative gate.

## Evidence required in your done-report
READY-FOR-HARVEST + branch + `git rev-parse HEAD` SHA (echo it) + validate-pr summary + a sample audit line showing the tier label + reviewer_venue field + test names added.

---
# EMBEDDED SSOT: ratified auto-merge tier expansion proposal (AUTOMERGE_TIER_EXPANSION_PROPOSAL_dev4_20260702.md)
