# DISPATCH BRIEF — dev-1 — ce-ops#294 press-merge evidence bundle (ratified demo build)
Date: 2026-07-03 · Role: implementer · Work class: S
Branch: `ce-294-press-merge-bundle` (off freshly-fetched origin/main)
You self-push and open the PR (you are the non-contained seat with push auth) — but see the merge stop-line.

## Mandate (ratified 2026-07-03, all 6 design asks approved; grant R-D covers the demo)
Build the press-merge evidence bundle v1 per the embedded ratified design (it is the SSOT; §5 is your file list):
1. `validators/creator_engine_validator/forge/press_merge_evidence.py` — pure assembler, JSON canonicalizer, source-provenance model, staleness-check helper. Read-only by construction.
2. v1 schema `ce-press-merge-evidence-bundle.v1.json` — MUST include the Tier-B ledger fields from day one (ratified addition): old/new record counts, old/new active counts, head hashes, superseded assertion IDs. Every field mapped to a named source with provenance (SHAs, run IDs). Derived `summary.verdict` (ready_for_human_merge / blocked / stale) explicitly NON-authority-bearing (label it so in the schema description).
3. Inert CLI renderer in `ce_cli.py` near the existing inert automerge commands.
4. `.github/workflows/automerge-decide.yml`: read-only assemble+upload step AFTER `Run automerge decision`, artifact name `ce-press-merge-evidence-bundle-*`. ⚠️ SHARED-FILE NOTICE: another in-flight lane is editing this file's env block. Make this workflow edit your LAST commit, `git fetch && git rebase origin/main` immediately before it, and keep your change to a strictly appended step.
5. `validators/tests/unit/test_press_merge_evidence.py` — source mapping, stale-head invalidation, approval-witness current-head logic, missing-optional validate-pr evidence, canonical JSON digest stability.
6. Strict head binding: bundle minted for exactly one head SHA, invalid on push; approval counted only when witness commit == bundle head.
7. Known gap, handle honestly: validate-pr output is not durably captured today → first bundles set `validation.available=false`. Do NOT fake it. (Durable capture is a ratified follow-up ticket, not yours.)
8. Demo: your own PR is the demo target — once the decide workflow runs on it, verify the bundle artifact is produced and well-formed, and include the artifact's summary JSON in your done-report.

## Hard never-list (from the ratified design — encode in code + workflow permissions)
No merge, no approve, no PR-body edits, no state mutation beyond publishing the artifact. Workflow step gets read-only permissions.

## STOP LINES
- Do NOT touch automerge_policy.py / automerge_actuator.py / mutation_classifier.py (another lane owns them today).
- Do NOT enable/flip anything; assembler + inert renderer only.
- Open the PR with: exactly one `- **Declared work class:** story`-style line per the G5 gate (declare the class that matches your diff), `.ce/changelog/ce-294-press-merge-bundle.md`, carrier regenerated via carrier_gen `write_carriers(base=<merge-base>)`. Run FULL `ce validate-pr` GREEN in ONE pass before push (standing directive ce-ops#303). You never approve or merge your own PR.

## Evidence required in your done-report
PR number + head SHA + validate-pr summary + bundle artifact name from the demo run + the bundle's summary block verbatim + any doc-vs-reality discrepancies in the design's cited line ranges.

---
# EMBEDDED SSOT: ratified press-merge evidence bundle design (PRESS_MERGE_EVIDENCE_BUNDLE_DESIGN_dev4_20260702.md)
