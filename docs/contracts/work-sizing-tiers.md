...
CE work classes (`tiny`, `story`, `feature`, `epic`) are CE ceremony tiers.
The names are borrowed nouns, not Agile or Scrum work item types. They do not
assert a user-story, feature, epic, sprint, backlog, or parent/child
relationship. A tier only selects the minimum decomposition depth, artifact
bundle, and review ceremony expected for governed CE work.
