...
| `docs/security/trust-anchors.md` | Independent public fingerprint record | Publish the successor fingerprint and an explicit transition or revocation status through every independent anchor channel. |
...
| `docs/install.sh` | Fetches the root and an out-of-band fingerprint anchor; refuses missing, mismatched, or same-origin anchors before installation | Point the bootstrap at the successor root and successor independent anchor, while preserving an intentional legacy path only when it remains trusted. |
...
| `docs/llms.txt` | Public entry point that names the root, fingerprint, and anchor model | Update the public discovery text and successor fingerprint only after its independent anchor is available. |
...
| `validators/creator_engine_validator/update.py` | Update path fetches the install spec, root endpoint, and independent anchor | Re-anchor its endpoint and anchor defaults to the successor. |
...
3. Before using the successor for a release, publish its public key and
   fingerprint in the trust-root file and independent anchor records. Confirm
   that each independent channel agrees on the successor fingerprint. A
   same-origin record is not an independent anchor.
...
5. Regenerate the current public release package. The controller alone signs
   ...
   public-root, independent-anchor, and consumer changes have been verified.
6. Reissue previous releases as versioned successor records. Each reissue
   ...
   release record. Publish the matching successor public key and independent
   anchor evidence with that record. Do not overwrite old release bytes or
   claim that an old signature changed signers.
...
independent fingerprint anchor. Existing `ce-root-v1`-only installers can keep
...
but it does not replace the required independent fingerprint anchor.
...
2. Publish the revocation through every independent public anchor channel and
   remove the old fingerprint from records that installers consult. Change the
   current trust-root endpoint and pin set so current consumers reject the old
   identity. A missing, mismatched, or unpinned root must refuse before any
   installation or release finalization proceeds.
3. Mint and independently anchor `ce-root-v2` under the custody rules above.
   Re-anchor every current consumer in the inventory, including the bootstrap,
   embedded pins, update path, release staging, release-finalize expectation,
   and release-smoke policy.
...
5. Verify that stale bootstrap attempts fail closed: the old identity must no
   longer satisfy both a current public root and an independent anchor. Verify
   that the successor path succeeds only with the successor key, expected
   namespace, canonical bytes, and an agreeing independent anchor.
...
IDs, public fingerprints, effective dates, independent-anchor confirmation,
