...
## 6. Deletion plan (ordered PR series)

The dependency graph is **one-way**: `hook_check.py` imports the *durable* checks (`completion_report_*`, `mutation_class`, `path_manifest_fidelity`); the durable checks do **not** import the doomed coordination/posture machinery. Deletion is a **localized excision** (~9% of the code, per the prior architect). Each PR carries a **closed manifest**, deletes matching tests + `docs/operations/` protocols + `schemas/` + `examples/` in the *same* PR, and lands under GitHub independent review.

**Ordered series (respecting the one-way graph; later PRs depend on earlier):**
