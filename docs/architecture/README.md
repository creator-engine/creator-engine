...
| [`stage-vocabulary.md`](./stage-vocabulary.md) | **The stage-vocabulary canon (#159).** The user-facing fractal cognitive phases `Frame → Shape → Build → Review → Ship` layered over the conserved mechanical state machine, the dual-mapping table (cognitive ↔ board ↔ spec-lifecycle ↔ gate ↔ BMAD phase), and the fractal/altitude framing. Consumed by G-6 (coordination) / G-7 (product surface). |
