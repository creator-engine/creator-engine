...
| [`stage-vocabulary.md`](./stage-vocabulary.md) | **The vocabulary canon (#159 + #162-era).** Both user-facing vocabularies over the conserved machine: the fractal stage phases `Frame → Shape → Build → Review → Ship`, and the Scope-card fields (`Goal · Done-when · Budget · Change-type · Ready`), each dual-mapped (label ↔ mechanical state/field), plus the fractal/altitude framing. Consumed by G-6 (coordination) / G-7 (product surface). |
