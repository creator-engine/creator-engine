...
| §b.3 Validation evidence | Commands and checks that ran, with skipped checks explicitly named. For release-track merges this includes the validator runs and content smoke checks the envelope declared. For a (future) deploy this would include the deploy-time checks Feature 006 declares; in v0.1 this field continues to cite local validation per [`./DEFINITION_OF_DONE.md`](./DEFINITION_OF_DONE.md) §c.4. |
