...
| v5 | 2026-06-08 | `index-v5-nvidia-ready.html` | `af6ee9d` (#174), tidy `df85fe0` (#176) | `9e799a47d59f2147847c30369ba92f169b5cc2979988becd4f82e2f55cd1eb50` | Consumer-facing redesign — a "two-altitude descent" from the consumer promise (idea→app, safely) down to the mechanism proof (phases, the gate, the grader, the ledger). Title "Turn your home computer into a software factory." Keeps Control-Room Violet (off-black + neon violet) and adds a neon-lime create/safe/go accent (lime→violet down the page); crimson stays the one human gate. New: a dedicated **Built for security with NVIDIA OpenShell** section (OpenShell isolates the runtime, CE governs the work — complementary infrastructure, not an NVIDIA product) and a consumer-worry→CE-mechanism safety-mapping table. Hero `ce session` runs a plain-language request → Scope card → human gate → ◆ Completion Report → PR, keeping the "representative" badge + roadmap link. Canon retained: Frame → Shape → Build → Review → Ship, the Scope card, the ◆ Completion Report, grader-outside-the-agent, agents-propose / human-ratifies. **v5.1 cosmetic tidy (#176):** synced `<title>`/`twitter:title` to the corrected H1, and demoted the duplicate "fear maps to a mechanism" headline to a sub-head (SHA updated above; no redesign, in-place patch — no new archive row). |
| **v6 (current — live)** | 2026-06-10 | `../docs/index.html` | this change | `c90688e9f12f8313e920971b5ee0c7e9d7c4dbdde0537a22901a1ce890ddb452` | Onboarding surface (v3.5-E.1+E.2) — no redesign, in-place addition to v5. Hero gains an **install box**: the copy-able one-liner `curl -fsSL https://creator-engine.dev/install.sh \| bash` + the agent-native pointer ("point your coding agent at `…/llms-install.md`"). In the same PR the installer artifacts moved to the Pages root (`docs/install/install.sh` → `docs/install.sh`, `docs/install/llms-install.md` → `docs/llms-install.md`) so the URLs the bootstrap prints resolve; NEW `docs/llms.txt` (agent-discovery index at `/llms.txt`); `llms-install.md` gains **§6 Connect to your collaboration repo** (two-mode: provided values or one batched ask). |
