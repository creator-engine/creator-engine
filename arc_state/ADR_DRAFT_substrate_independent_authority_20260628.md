...
evidence_refs:
  ...
  - kind: issue
    ref: "ce-ops#341 (CLOSED) — AutoReview never-APPROVE guard: parameterize by run_mode for strangeLoop autonomy"
    tag: ce-ops-341
  - kind: issue
    ref: "ce-ops#6 — Lane: strangeLoop C-cluster, coordination and governed autonomy"
    tag: ce-ops-6
  - kind: issue
    ref: "ce-ops#291 (CLOSED) — CEO-mode policy-tiered auto-merge (Wave 1.1)"
    tag: ce-ops-291
  - kind: issue
    ref: "ce-ops#289 (CLOSED) — Egress broker: attest self-push socket origin via SO_PEERCRED"
    tag: ce-ops-289
  - kind: code
...
- **ce-ops#341** (closed) identified the never-APPROVE guard as a hardcoded constant that
  should instead flow from the run-mode, and noted that strangeLoop may need APPROVE
  under a deliberate governance decision.
- **ce-ops#291** (closed, CEO-mode) built the policy-tiered auto-merge run-mode engine
  (dry-run mode), which parameterizes what is allowed without a per-PR human gesture.
- **ce-ops#6** (strangeLoop lane) is the long-pole autonomous-review endpoint: a governed
...
- The run-mode parameterization from ce-ops#341 and the reviewer-authority-envelope from
  G2.007.2/G2.007.3 are already the right primitives; they need wiring, not redesign.
...
3. **Run-mode policy check (required for APPROVE).** The active run-mode must permit
   ...
   For a future explicitly-ratified `strangeLoop` run-mode (ce-ops#6), the loop itself
   constitutes the governance path, and APPROVE may be permitted under the envelope.
...
- Unblocks agent-autonomous review under ratified run-modes (the ce-ops#6 strangeLoop
  endgame and the near-term CEO-mode auto-review-and-approve path).
...
- The run-mode integration must be tested before any live APPROVE is permitted: dry-run
  classify-only mode must gate the first live flip (same discipline as ce-ops#291).
...
| ce-ops#341 (run_mode parameterization) | This ADR ratifies the principle that #341 identified: the never-APPROVE guard must be a run-mode-keyed policy, not a constant. This ADR is the written decision; #341 surfaced the gap. |
| ce-ops#291 (CEO-mode) | The CEO-mode policy engine provides the run-mode infrastructure. APPROVE permission in a given run-mode is a policy decision encoded in that engine. |
| ce-ops#6 (strangeLoop) | strangeLoop is the autonomous-review endpoint. The APPROVE permission under strangeLoop is an explicit governance decision to be made when strangeLoop is scoped; this ADR clears the way for it. |

## What this ADR does NOT decide

- The definition of the strangeLoop run-mode or the specific conditions under which APPROVE
  is permitted within it. That is ce-ops#6.
...
This is a draft ADR awaiting Operator ratification (ce-ops#348). Its substrate-independence
body (D2/D3) was drafted by a governed worker under the Operator-ratified principle; the
action-taxonomy (D1) was authored by the Orchestrator under direct Operator direction on
2026-06-28 (the decision to make authority action-centric rather than grant-numbered). It
records the ratified principles and identifies the implementation gap. It does not authorize
any live code change, any APPROVE submission, or any weakening of the current default-deny
posture. On ratification it promotes to `docs/decisions/ADR-0013-*.md` via a governance PR
(verify no overlap with the private ce-ops ADR-0003/ADR-0004 first); the broker/proxy
modification it motivates is the separate ratified implementation ticket (ce-ops#349), which
requires its own full preflight pass before merging.
