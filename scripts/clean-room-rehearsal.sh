...
CE_REHEARSAL_PLAN stage=first_value action="scripts/first-value.sh author->commit->push->PR->merge" guarded=TODO fail_closed=1
...
run_first_value() {
  ...
  need_container "${stage}"
  fail "${stage}" "TODO guarded: future N3 scripts/first-value.sh author->commit->push->PR->merge is not implemented in this harness yet"
}
