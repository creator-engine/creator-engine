...
| `role` | string | yes | enum `architect_research`, `implementer`, `verification`, `controller` | Seat role enum. The three runtime seats carried from the v2 worker-container contract plus the controller role ratified for launch-default policy records. Additional roles MUST be ratified as additive schema changes. |
