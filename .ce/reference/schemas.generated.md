...
| `model_effort_line` | string | yes | pattern `^model=[^=;\\s]+; effort=(medium\|high\|xhigh)$` | Canonical launch-policy stamp, generated from the resolved wrapper model/effort pair: "model=gpt-5.6-terra; effort=high". It is not free prose: the receipt builder validates it against the same policy that clamps low... |
| `model_effort_note` | string | no | const `unverified-attached-session: --resume did not recreate or reassert model/effort` | Optional, bounded observation note. Present only when a dispatcher attached to an existing session, whose recorded model/effort cannot be verified or reasserted by the wrapper. Watchers must treat this as an attachmen... |
...
| `role` | string | yes | enum `architect_research`, `implementer`, `verification`, `controller` | Seat role enum. The three runtime seats carried from the v2 worker-container contract plus the controller role ratified for launch-default policy records (ce-ops#447, Operator-ratified 2026-07-05 day-arc). Additional... |
| `isolation_backend` | string | no | enum `gvisor-proxy`, `docker`, `openshell`, `local-noop`, `os-native` | Runner-backend selector (the G-1.1 adapter axis). `gvisor-proxy` is the canonical v3 backend — a hardened gVisor container paired with a capability-separation egress proxy (shipped at G-1.2). `docker` is the plain-Doc... |
