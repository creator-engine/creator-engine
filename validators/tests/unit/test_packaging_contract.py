...
def test_pages_mirror_sha256s_publishes_install_sh_and_wheels(repo_root: Path):
    ...
    # in-place under docs/downloads/0.2.0/ -- NOT the live docs/install.sh or
    # validators/wheelhouse/. Together with the test above this pins a bijection
    # between the mirror's wheel files and its SHA256SUMS wheel entries, so
    # neither an orphan file nor an unpublished wheel can slip in.
    mirror = repo_root / "docs" / "downloads" / "0.2.0"
    sums = _parse_sha256sums(mirror / "SHA256SUMS")
    install_sh = mirror / "install.sh"
    assert install_sh.is_file(), "published mirror must contain install.sh"
    assert sums["install.sh"] == _sha256(install_sh)
