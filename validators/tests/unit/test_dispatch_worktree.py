"""Offline contract tests for the boundary-neutral dispatch-worktree core."""
...
class FakeDispatchPrimitives:
    ...
    def acquire_work_claim(self, key, **_kwargs):
    ...
    def allocate_worktree(self, **kwargs) -> None:
    ...
    def scrub_worker_environment(self, **kwargs):
        lane_id = str(kwargs["worker_id"])
        self.calls.append(("scrub", lane_id, None))
        return {"CE_FAKE_WORKER": lane_id}, ("TOKEN",)
    ...
    def release_worktree(self, **kwargs) -> None:
        lane_id = str(kwargs["lane_id"])
    ...
    def release_work_claim(self, key, **kwargs):
    ...
    def best_effort_release_work_claim(self, key, claim_id, **kwargs) -> bool:
        reason = kwargs.get("reason")
...
def _dispatch(spec: dw.DispatchSpec, fake: FakeDispatchPrimitives) -> dw.DispatchOutcome:
    return dw.dispatch(
        spec,
        primitives=fake,
...
def test_dispatch_worktree_imports_no_v1_runtime_modules():
    ...
    assert "pco_allocator" not in imported
    assert "worker_spawn" not in imported
    assert "runtime_backend_bridge" not in imported


def test_happy_path_claim_wins_exec_pushes_and_releases_in_order():
    fake = FakeDispatchPrimitives(claim_id="claim-happy")
...
def test_foreign_claim_refuses_before_worktree_or_exec():
    fake = FakeDispatchPrimitives(claim_ok=False)
...
def test_two_specs_with_different_work_keys_get_distinct_paths_and_branches():
    fake = FakeDispatchPrimitives()
...
def test_exec_rc_one_aborts_without_push_and_releases_as_aborted():
    fake = FakeDispatchPrimitives(exec_rc=1)
...
def test_allocate_exception_best_effort_releases_claim_without_exec():
    fake = FakeDispatchPrimitives(claim_id="claim-allocate", allocate_raises=True)
...
def test_exec_exception_still_runs_releases_and_duplicate_release_is_noop():
    fake = FakeDispatchPrimitives(exec_raises=True)
...
def test_long_work_key_mints_pattern_valid_lane_id_no_longer_than_64_chars():
    fake = FakeDispatchPrimitives()
