...
runner sandbox. It does not harden absolute binary paths, bundled binaries,
libgit2/JGit, raw HTTPS API clients, PATH resets, environment/posture reset
(e.g. ``CE_RING1_POSTURE``), or arbitrary filesystem syscalls. Those
escape-hardening layers, including posture-resolution hardening, are the next
gate.
...
class Ring1ToolGuardConfig:
    ...
    def __post_init__(self) -> None:
        if self.posture not in {"auto", "governed", "ungoverned"}:
            raise ValueError("posture must be 'auto', 'governed', or 'ungoverned'")
        if not self.tools:
...
def render_posix_tool_shim(
    ...
    constants = {
        ...
        "default_posture": config.posture,
        "default_evidence_root": config.evidence_root,
        "posture_env": config.posture_env,
        "posture_root_env": config.posture_root_env,
        "evidence_root_env": config.evidence_root_env,
        "ledger_root_env": config.ledger_root_env,
...
# PATH resets, environment/posture reset (e.g. `CE_RING1_POSTURE`), and
# non-shell filesystem access; hardening is a later gate.
set -eu
...
posture = os.environ.get(CFG["posture_env"], CFG["default_posture"])
evidence_root = os.environ.get(CFG["evidence_root_env"], CFG["default_evidence_root"])
posture_root = os.environ.get(CFG["posture_root_env"]) or os.getcwd()
...
ledger_root = os.environ.get(CFG["ledger_root_env"])
if ledger_root:
