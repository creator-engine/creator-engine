...
def _app_wheel_source_violations(root: Path, wheels: Iterable[Path]) -> list[str]:
    ...
    if pyproject_version and source_version and pyproject_version != source_version:
        v.append(f"pyproject version {pyproject_version!r} differs from creator_engine_validator.version.__version__ {source_version!r}")
    source_files = {p.relative_to(source_root).as_posix(): p for p in source_root.rglob("*.py") if p.is_file()}
    for wheel in wheels:
        ...
        if wheel_dist != DISTRIBUTION_NAME:
            v.append(f"app wheel {wheel.name} distribution must be {DISTRIBUTION_NAME!r}, got {wheel_dist!r}")
        if pyproject_version and wheel_version != pyproject_version:
            v.append(f"app wheel {wheel.name} version {wheel_version!r} differs from pyproject version {pyproject_version!r}")
        if source_version and wheel_version != source_version:
            v.append(f"app wheel {wheel.name} version {wheel_version!r} differs from creator_engine_validator.version.__version__ {source_version!r}")
        try:
            with zipfile.ZipFile(wheel) as zf:
                wheel_files = {n.removeprefix("creator_engine_validator/") for n in zf.namelist() if n.startswith("creator_engine_validator/") and n.endswith(".py")}
...
def verify_wheel_matches_source(repo_root: Path | str, *, build_app_wheel=build_app_wheel_from_source, wheel_bake_error: type[BaseException] = WheelBakeError) -> list[str]:
    root = Path(repo_root)
