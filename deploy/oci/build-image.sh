...
Defaults:
  IMAGE:    creator-engine/ce-validator:0.3.5
...
Run the built image:
  docker run --rm creator-engine/ce-validator:0.3.5 ce --help
  docker run --rm -v "$PWD:/workspace/creator-engine" -w /workspace/creator-engine \
    creator-engine/ce-validator:0.3.5 creator-engine-validator check-examples
USAGE
}
...
image="${CE_OCI_IMAGE:-creator-engine/ce-validator:0.3.5}"
