...
Defaults:
  IMAGE:    creator-engine/ce-validator:0.3.6
...
Run the built image:
  ...
  docker run --rm -v "$PWD:/workspace/creator-engine" -w /workspace/creator-engine \
    creator-engine/ce-validator:0.3.6 creator-engine-validator check-examples
USAGE
}
...
image="${CE_OCI_IMAGE:-creator-engine/ce-validator:0.3.6}"
...
print_stage_context_commands() {
  ...
  quote_cmd cp "${repo_root}/validators/requirements.txt" "${context_dir}/validators/requirements.txt"
  quote_cmd cp "${repo_root}/validators/requirements-dev.txt" "${context_dir}/validators/requirements-dev.txt"
...
stage_context() {
  ...
  cp "${repo_root}/validators/requirements.txt" "${context_dir}/validators/requirements.txt"
  cp "${repo_root}/validators/requirements-dev.txt" "${context_dir}/validators/requirements-dev.txt"
